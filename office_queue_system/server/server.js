...
app.get("/api/officers", async (req, res) => {
  try {
    ...
    res.status(500).json({
      code: 500,
      error: `Database error during the retrieve of the list of officers.`,
    });
  }
});

//get services per desks
app.get("/api/servicesperdesk", async (req, res) => {
  try {
    ...
    res.status(503).json({
      code: 503,
      error: `Unavailable service error during the retrieve of the services associated to a desk`,
    });
  }
});
//get services
app.get("/api/services", async (req, res) => {
  try {
    ...
    res.status(503).json({
      code: 503,
      error: `Unavailable service error during the retrieve of the services `,
    });
  }
});
...
app.post("/api/serveNextTicket", async (req, res) => {
  ...
  console.log(old_ticket_id);
  console.log(new_ticket_id);
  const desk_id = req.body.desk_id;
  var outcome;

  try {
    outcome = await ticketsDao.serveNextTicket(
      old_ticket_id,
      new_ticket_id,
      desk_id
    );
...
app.delete("/api/serviceForDesk/delete/:deskId/:serviceId", (req, res) => {
  ...
  adminDao
    .deleteServiceOfDesk(desk_id, service_id)
    .then((id) =>
      res
        .status(204)
        .json(
          `Selected service with id:${service_id} for Desk: ${desk_id} was deleted`
        )
    )
    .catch((err) =>
      res
        .status(500)
        .json(
          `Error while deleting the survey with id:${req.params.id}  ` + err
        )
    );
});
